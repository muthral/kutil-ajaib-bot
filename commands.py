...
from data import (
    chat_members, recent_chatters, MAX_RECENT, get_nama, format_rupiah, get_raw_name,
    game_sessions, chaos_sessions, duel_sessions, duel_dm_pending, spy_guess_pending,
    get_scores_dict
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.chat.type == "private":
        await update.message.reply_text("selamat datang di bot kutil ajaib\n\ngunakan /help untuk melihat command.")
...
async def apa(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("masukkan pertanyaannya"); return
    pertanyaan = " ".join(context.args).lower()
    if context.args[0].lower() == "kabar":
        await update.message.reply_text("baik"); return
    responses = []
...
async def hitung(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("masukkan pertanyaan"); return
    pertanyaan = " ".join(context.args).lower()
    angka = random.randint(0, 100)
    await update.message.reply_text(f"{angka}%" if "persen" in pertanyaan else str(angka))

async def tagrandom(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.message.chat_id
    members = chat_members.get(chat_id, {})
    if not members:
        await update.message.reply_text("belum ada member terdeteksi. chat dulu biar kedeteksi!"); return
...
async def tag7(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not recent:
        await update.message.reply_text("belum cukup member terdeteksi!"); return
    seen_ids, kandidat = set(), []
    for ts, user in reversed(list(recent)):
        if user.id not in seen_ids:
            seen_ids.add(user.id); kandidat.append(user)
    for uid, user in chat_members.get(chat_id, {}).items():
        if uid not in seen_ids:
            seen_ids.add(uid); kandidat.append(user)
    if not kandidat:
        await update.message.reply_text("tidak ada member terdeteksi"); return
    terpilih = random.sample(kandidat, min(7, len(kandidat)))
    mentions = [f"@{u.username}" if u.username else f'<a href="tg://user?id={u.id}">{u.first_name}</a>' for u in terpilih]
...
async def tag2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    members = chat_members.get(chat_id, {})
    if len(members) < 2:
        await update.message.reply_text("belum ada cukup member terdeteksi!"); return
    terpilih = random.sample(list(members.values()), 2)
    mentions = [f"@{u.username}" if u.username else f'<a href="tg://user?id={u.id}">{u.first_name}</a>' for u in terpilih]
    await update.message.reply_text(f"🎯 {mentions[0]}, {mentions[1]}", parse_mode="HTML")

pesan_jodoh = [
    "semoga samawa ya!", "jodoh banget, gak diragukan.",
    "meski nampaknya seperti cinta bertepuk sebelah tangan 🤔",
    "bgus, kpn ciuman?", "pasangan ini benar-benar saling mencintai 🪽"
]
...
async def skor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await update.message.reply_text("belum ada yang main atau belum ada yang menang"); return
